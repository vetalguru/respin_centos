...
ISONAME="${DISTR_NAME} ${DISTR_VERSION} ${DISTR_MACHINE}"  # need to change it in isolinux/isolinux.cfg
ISODATE="$(date +'%d/%m/%y')"
...
if [[ ! -d "${FULL_RESPIN_RPMS_DIR_PATH}" ]]; then
    ...
    echo "Build the ${DISTR_NAME} was STOPPED!!!!"
...
chmod +x ${FULL_BUILD_DIR_PATH}/isolinux/isolinux.cfg

sed -i "s/CentOS/${DISTR_NAME}/g" ${FULL_BUILD_DIR_PATH}/isolinux/isolinux.cfg
